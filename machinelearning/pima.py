...
Note: If inside a virtualenv, run Pima class within a jupyter notebook in order
      to get around Matplotlib Framework issues.
"""
...
class Pima(object):
    ...
    def _prepare(self):
        ...
        column_names = ["n_preg", "plasma", "blood_pressure", "triceps",
                        "insulin", "bmi", "dpf", "age", "label"]
    ...
    def grid_search(self, param_grid, scoring=None, cv=3):
        """
        Run grid-search with self.estimator through param_grid parameter space.
        """
    ...
    def experience_curve(self, train_sizes=None, cv=3, ylim=None,
                         scoring="roc_auc"):
        """
        Return matplotlib plt object with learning/experience curve using
        self.estimator.
        """
        ...
        train_sizes, train_scores, test_scores = learning_curve(
            self.estimator, X, y, cv=cv, n_jobs=-1, train_sizes=train_sizes,
            scoring=scoring)
    ...
    def plot_roc_curve(self):
        ...
        probas_ = self.estimator.fit(X_train, y_train).predict_proba(X_test)
...
def forest_example():
    ...
    estimator = RandomForestClassifier(max_features=0.3, min_samples_split=2,
